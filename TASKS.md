...
- [-] Task 6 - Verify, publish, and close #1295
  ...
  - Check:
    ...
    - The subsequent run passed every backend, image, quickstart, and policy job before one homepage Playwright assertion observed both the streamed `Syncing` fallback and settled `Relay ready` hero during Suspense reveal. The test now waits for a settled relay state before requiring one hero; the focused regression passed 20/20 repeated Chromium runs.
