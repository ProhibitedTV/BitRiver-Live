...
3. Copy the viewer files from the extracted release into `/opt/bitriver-viewer`:
   ...
   ```
   Adjust the paths if you staged the release elsewhere. The systemd unit executes `node .next/standalone/server.js`, so keep the entire standalone bundle intact when you sync files. When running the container image, mount `/opt/bitriver-viewer` as a volume instead of copying files.
