...
func useStubFFmpeg(t *testing.T) string {
	...
	if runtime.GOOS == "windows" {
		stubPath = writeWindowsFFmpegStub(t)
	}
	...
	if !sameExecutablePath(resolved, stubPath) {
...
func TestJobProducesSegmentsAndCanBeStopped(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("stat live mirror: %v", err)
	}
	if runtime.GOOS != "windows" && info.Mode()&os.ModeSymlink == 0 {
		t.Fatalf("expected live mirror to be a symlink, got %s", info.Mode())
	}
	if runtime.GOOS != "windows" {
		target, err := filepath.EvalSymlinks(liveLink)
		if err != nil {
			t.Fatalf("resolve live mirror target: %v", err)
		}
		...
		if !sameExecutablePath(resolvedTarget, expectedTarget) {
			t.Fatalf("unexpected live mirror target: %s", resolvedTarget)
		}
	}
	...
	mirrorCleanupTimeout := 5 * time.Second
	if runtime.GOOS == "windows" {
		mirrorCleanupTimeout = 15 * time.Second
	}

	waitFor(t, mirrorCleanupTimeout, "expected live mirror to be removed", func() bool {
	...
	waitFor(t, mirrorCleanupTimeout, "expected cancelled job mirror cleanup", func() bool {
