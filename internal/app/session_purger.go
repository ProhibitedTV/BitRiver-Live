...
type sessionPurger interface{ PurgeExpired() error }
...
func startSessionPurgeWorker(ctx context.Context, logger *slog.Logger, sessions sessionPurger, interval time.Duration) func() {
	return startSessionPurgeWorkerWithTicker(ctx, logger, sessions, interval, func(d time.Duration) purgeTicker { return timeTicker{ticker: time.NewTicker(d)} })
}

func startSessionPurgeWorkerWithTicker(ctx context.Context, logger *slog.Logger, sessions sessionPurger, interval time.Duration, newTicker tickerFactory) func() {
