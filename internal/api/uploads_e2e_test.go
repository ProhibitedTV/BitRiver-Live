...
func TestE2EUploadMultipartAppearsInList(t *testing.T) {
	...
	uploadResp := postMultipartUpload(t, server.URL+"/api/uploads", channel.ID, "tiny.mp4", validMP4Sample())
	...
	listReq, err := http.NewRequest(http.MethodGet, server.URL+"/api/uploads?channelId="+channel.ID, nil)
	if err != nil {
	...
	if len(listed) == 0 {
		t.Fatal("expected at least one upload in listing")
	}
	if listed[0].ID != uploadResp.ID {
		t.Fatalf("listed upload id = %q, want %q", listed[0].ID, uploadResp.ID)
	}
}

func postMultipartUpload(t *testing.T, endpoint, channelID, filename string, fixture []byte) uploadResponse {
	t.Helper()
