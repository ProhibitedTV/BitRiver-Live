...
def _replace_access_token(text: str, token: str) -> str:
    token = xml_escape(token)
    match = re.search(r"<AccessTokens>(.*?)</AccessTokens>", text, re.DOTALL)
    if match:
        start, end = match.span()
        inner = replace_tag_content(match.group(1), "AccessToken", token)
        return text[:start] + f"<AccessTokens>{inner}</AccessTokens>" + text[end:]

    if "<AccessToken>" in text:
        return replace_tag_content(text, "AccessToken", token)

    raise SystemExit("missing <AccessTokens> or <AccessToken> in template")


def _replace_authentication(text: str, username: str, password: str) -> str:
    auth_match = re.search(r"<Authentication>(.*?)</Authentication>", text, re.DOTALL)
    if not auth_match:
        raise SystemExit("missing <Authentication> block in template")

    start, end = auth_match.span()
    auth_body = auth_match.group(1)

    auth_body = replace_tag_content(auth_body, "ID", xml_escape(username))
    auth_body = replace_tag_content(auth_body, "Password", xml_escape(password))

    return text[:start] + f"<Authentication>{auth_body}</Authentication>" + text[end:]


def _stamp_image_tag(text: str, image_tag: str | None) -> str:
    if not image_tag:
        return text

    marker = f"<!-- Rendered for BITRIVER_OME_IMAGE_TAG={xml_escape(image_tag)} -->"
    if "<Server" not in text:
        return text

    pattern = re.compile(r"<!--\s*Rendered for BITRIVER_OME_IMAGE_TAG=.*?-->")
    if pattern.search(text):
        return pattern.sub(marker, text, count=1)

    return text.replace("<Server version=\"10\">", f"<Server version=\"10\">\n    {marker}", 1)
...
def render(
    ...
    text = _replace_access_token(text, access_token or api_token)
    text = _replace_authentication(text, username, password)
    text = _stamp_image_tag(text, image_tag)
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--username",
        required=True,
        help="Authentication username injected into <Authentication><User><ID>",
    )
    parser.add_argument(
        "--password",
        required=True,
        help="Authentication password injected into <Authentication><User><Password>",
    )
    parser.add_argument(
        "--api-token",
        required=True,
        help="Managers API token rendered inside <AccessTokens>",
    )
    parser.add_argument(
        "--access-token",
        required=False,
        help="Access token forwarded to health probes; defaults to --api-token",
    )
    parser.add_argument(
        "--image-tag",
        required=False,
        help="OME image tag recorded in the render marker",
    )
