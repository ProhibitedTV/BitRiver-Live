...
- `deploy/install/compose-host.sh` and `deploy/systemd/bitriver-live-compose.service`
  ...
  - The installer persists `BITRIVER_CONFIG_ROOT=/etc/bitriver-live` plus the numeric non-root Docker operator in `BITRIVER_HOST_UID`/`BITRIVER_HOST_GID`; the packaged unit supplies the same values. Direct and systemd Compose paths therefore resolve the installed workspace's absolute `.env`, OME, and SRS symlinks and run only bind-writing services as the owner of private config/data. Installed generated output uses the same source-shaped tree as Compose: `/etc/bitriver-live/deploy/ome/Server.generated.xml` and `/etc/bitriver-live/deploy/srs/conf/srs.generated.conf`. Older flat generated files migrate byte-for-byte with compatibility links; divergent dual copies are rejected before new program assets or operator configuration are staged. Older env files gain the managed keys on upgrade; duplicate active entries are rejected before any operator configuration is rewritten.
  - `srs-config`, `ome-config`, and `ome-health-token-check` mount program assets read-only. The two renderers write operator-owned, group-readable mode-`0640` generated output directly through `/etc/bitriver-live/deploy` on the separate config-root bind, and the verifier reads the same path; the SRS helper executes its CRLF-sanitized copy from container tmpfs. OME and SRS receive only the selected operator GID as a supplementary group so they can read those files while keeping all capabilities dropped and the mounts read-only. The private environment remains mode `0600`.
- Generated files (verified in repository code paths)
  - `deploy/ome/Server.generated.xml`
    ...
    - The tracked repository file is rendered from `deploy/.env.example` and contains placeholders only. Release archives/packages exclude it and seed `/etc/bitriver-live/deploy/ome/Server.generated.xml` from `Server.xml`; deployment-time output is sensitive and must never be uploaded as CI or release evidence.
  - `deploy/srs/conf/srs.generated.conf`
    ...
    - This file is not committed in this repository and is generated from `deploy/srs/conf/srs.conf` + `.env` by `scripts/render-srs-config.sh`. Packaged installs persist it at `/etc/bitriver-live/deploy/srs/conf/srs.generated.conf`. In Compose-first flows, `srs-config` always runs with `--force` before `srs` starts; in packaged systemd flow, `srs.service` also runs the same render script with `--force` in `ExecStartPre`.
