...
Operators can use the manifests under `deploy/` as a reference architecture for production or staging hosts. The current public candidate also publishes a source-free Ubuntu package and launcher archive for the same Compose contract; follow the checksum and signed-release-set path in the [Ubuntu guide](installing-on-ubuntu.md). Do not substitute a source checkout for the artifact-only host qualification required before stable promotion.
