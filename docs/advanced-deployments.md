...
| `BITRIVER_OME_BIND` | Address written to the `<IP>` and `<Bind>` fields in `Server.xml` (defaults to `0.0.0.0`). |
...
OME 0.15.10 requires a `<Bind>` tag at the top level, so the template rewrites both `<IP>` and `<Bind>` from `BITRIVER_OME_BIND` to avoid repeated schema errors when the container restarts.
