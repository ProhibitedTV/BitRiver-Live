...
- **OvenMediaEngine output:** `deploy/ome/Server.xml` keeps LL-HLS enabled for the `live` application by default. The Quickstart templating in `scripts/quickstart.sh` rewrites bind addresses/ports from `BITRIVER_OME_*` (including `BITRIVER_OME_LLHLS_PORT` / `BITRIVER_OME_LLHLS_TLS_PORT`) and mounts the generated `Server.generated.xml` into the OME container. HLS/DASH clients should read from the LL-HLS publisher on port `8080` (or `BITRIVER_OME_LLHLS_PORT` after templating) to reach the symlinked `public/live/<job>/index.m3u8` manifests produced by the transcoder. For Docker Compose, publish the LL-HLS listener via `BITRIVER_OME_LLHLS_HOST_PORT` to avoid colliding with `BITRIVER_LIVE_PORT` on the host.
