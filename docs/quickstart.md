...
- **`[Config] Unknown item found: Server.bind.managers.api.AccessTokens`** – The rendered `Server.xml` still contains the managers `<AccessTokens>` block even though your `BITRIVER_OME_IMAGE_TAG` points to a build that doesn't support it (for example, anything older than 0.16.0). Set `BITRIVER_OME_API_TOKEN` to an empty value in `.env`, rerun `./scripts/render-ome-config.sh --force`, and restart the stack so the regenerated config drops the unsupported tag before Docker brings OME back up.
