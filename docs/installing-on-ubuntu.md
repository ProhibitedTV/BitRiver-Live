...
Upgrades from candidates that stored generated output directly at
`/etc/bitriver-live/Server.generated.xml` or
`/etc/bitriver-live/srs.generated.conf` move a sole legacy file byte-for-byte
into the source-shaped tree and leave a compatibility link at the former path.
