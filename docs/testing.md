...
- **Source-free disaster-recovery foundation:** `./scripts/test-disaster-recovery.sh`
  ...
  - Set `BITRIVER_DISASTER_RECOVERY_ARTIFACT_DIR` to retain secret-scanned `bitriver.host-restore-report/v1`, `bitriver.postgres-restore-report/v1`, and `bitriver.disaster-recovery/v1` evidence. Local staging keeps exact published-package qualification explicit rather than treating checkout bytes as publication proof.
