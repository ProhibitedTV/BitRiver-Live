...
- **Published-package disaster recovery:** `./scripts/test-published-disaster-recovery.sh --release TAG --source-commit SHA --artifact-dir DIR`
  - Downloads the public Linux amd64 launcher and `release-set.json`, requires an empty evidence directory, validates the supported repository/tag/commit plus exact package name/size/SHA-256 before safe archive extraction, and then runs the complete lost-host drill with the recovery scripts from that package.
  - The retained report records the release-set/package hashes and explicitly leaves `releaseSetSignatureVerified` false. Pair it with the same candidate's release/clean-host Sigstore evidence; recovered-stack production golden path and production-like scheduled off-host RPO remain separate #1299 acceptance.
