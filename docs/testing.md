...
- **Source-free disaster-recovery foundation:** `./scripts/test-disaster-recovery.sh`
  - Requires Docker plus network access. It exports the real non-empty Postgres rehearsal set, stages only source-free package assets, deletes its disposable source host, reinstalls a fresh packaged-host layout, and restores Postgres through a disposable read-only helper using the same exact-digest Postgres 15 image as the unmodified database container before comparing local/external object invariants.
  - Set `BITRIVER_DISASTER_RECOVERY_ARTIFACT_DIR` to retain secret-scanned `bitriver.host-restore-report/v1`, `bitriver.postgres-restore-report/v1`, and `bitriver.disaster-recovery/v1` evidence, including the exact restore-helper image. Local staging keeps exact published-package qualification explicit rather than treating checkout bytes as publication proof.
- **Published-package disaster recovery:** `./scripts/test-published-disaster-recovery.sh --release TAG --source-commit SHA --artifact-dir DIR`
  - Downloads the public Linux amd64 launcher and `release-set.json`, requires an empty evidence directory, validates the supported repository/tag/commit plus exact package name/size/SHA-256 before safe archive extraction, and requires the exercised bundle's complete path/content inventory to match the verified archive subtree before any host mutation.
  - The retained report records the release-set/package hashes plus the exercised-bundle content fingerprint and explicitly leaves `releaseSetSignatureVerified` false. Pair it with the same candidate's release/clean-host Sigstore evidence.
- **Recovered immutable-stack golden path:** `./scripts/test-recovered-stack-golden-path.sh --release TAG --source-commit SHA --release-set-sha256 SHA256 --artifact-dir DIR`
  - Requires Docker and network access plus an otherwise idle host. It derives a private product-shaped environment from the verified public release set, creates a migrated non-empty backup, runs the exact package's destructive recovery, restores the same archive into the recovered installation, and starts only digest-pinned pull-only services. Backup and runtime restore run in disposable read-only helpers using the exact Postgres image; neither exercised database container receives packages or copied scripts.
  - The secret-scanned completion report requires matching original/runtime Postgres archive identity, the exact restore-helper image, 13 exact service image references, preserved recovered fixtures and additional persisted golden-path state, all eight passing product stages, and measured total RTO. The environment and sentinels remain temporary; scheduled/off-host freshness remains separate #1299 acceptance.
