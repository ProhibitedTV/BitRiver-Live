...
- **Published-package disaster recovery:** `./scripts/test-published-disaster-recovery.sh --release TAG --source-commit SHA --artifact-dir DIR`
  ...
  - The retained report records the release-set/package hashes plus the exercised-bundle content fingerprint and explicitly leaves `releaseSetSignatureVerified` false. Pair it with the same candidate's release/clean-host Sigstore evidence; recovered-stack production golden path and production-like scheduled off-host RPO remain separate #1299 acceptance.
