...
- Preserve one full Chromium desktop regression project as the broad baseline, then add a smaller release-critical matrix across Chromium, Firefox, WebKit, Android/Chrome emulation, and iPhone/WebKit emulation. Reuse existing channel/chat/playback, auth/navigation, creator setup, mobile overflow, and accessibility tests rather than building a parallel fake viewer.
- Separate engine/device support from protocol support. Mocked browser tests can prove layout, navigation, auth/cookie behavior, chat UI/reconnect presentation, player fallback/error states, and creator flows; real HLS/LL-HLS/WebRTC decode evidence remains owned by the production golden path and must not be inferred from mocked routes.
- Keep retry policy honest: do not add broad retries to turn browser-specific failures green. Any unsupported engine behavior must be fixed, narrowly excluded with documented rationale, or surfaced as a release blocker.
- Publish a support matrix that names the tested desktop/mobile projects, which critical flows run on each, browser-version policy, media/protocol caveats, and the boundary between CI emulation and real device/Safari evidence.
- Keep the release feature freeze intact: this slice may make compatibility fixes required by the declared baseline, but it does not add 4K support, new media protocols, or unrelated viewer features.

### Risks and test boundaries

- Running every existing browser test five times would be expensive and can obscure engine-specific failures behind unrelated coverage. Preserve the current complete suite on Chromium and define one focused compatibility spec set that runs on all five projects.
- Playwright's mobile presets can change user agent, viewport, touch, and device scale behavior. Ensure project-level emulation does not fight tests that intentionally resize through several narrow widths; mobile critical coverage should assert touch-capable behavior and no document overflow without hard-coding one device-only layout.
- HLS support differs by browser engine. Tests must distinguish native HLS, hls.js fallback, and unsupported/mock-only behavior instead of pretending a successful mocked playlist proves decoder support.
- WebKit on Linux is Safari-equivalent engine coverage, not a claim that current macOS/iOS Safari hardware was exercised. Documentation must state that boundary explicitly.
- The permanent CI path should install only browsers actually required by the matrix and retain one production standalone build/server boundary. Avoid duplicate Next builds per project.

### Test and rollout plan

- Add named Playwright projects for Chromium desktop, Firefox desktop, WebKit desktop, Android/Chrome emulation, and iPhone/WebKit emulation using Playwright's maintained device descriptors where appropriate.
- Add/retag a compact critical compatibility suite covering channel watch/playback recovery presentation, authentication/navigation, chat, creator setup, accessibility, mobile/touch interaction, and horizontal-overflow refusal.
- Add static repository contract tests for the project names, browser engines/device presets, no blanket retry policy, and expected critical/full-suite split.
- Update viewer/testing/operator documentation with the support matrix, browser-version policy, protocol caveats, and real-media follow-up requirement.
- Run clean npm install, zero-high audit, strict lint, Jest, all critical projects, full Chromium integration, production build, literal `./scripts/verify.sh --viewer`, protected viewer CI, automated review, and the aggregate Merge gate before squash merge.
- Keep #1307 open if real-media engine-class evidence or real Safari/device evidence remains unproven after this repository-side matrix lands.


## Current scope - scheduled off-host backup freshness qualification (#1299) (2026-09-12)

- Finish #1299's remaining repository-solvable scheduled/off-host slice by qualifying the existing `bitriver.postgres-backup/v1` objects emitted by `scripts/backup-postgres.sh`; do not add a second producer, scheduler, or remote-storage format.
- Add one read-only S3-compatible verifier that fails closed on incomplete remote trios, stale newest evidence, insufficient retained complete sets, checksum/manifest mismatch, canonical manifest-shape violations, or a backup that does not match the exact candidate release and full source commit.
- Mirror the restore-side v1 manifest contract: archive identity/size/hash, database/server identity, non-empty applied migration entries with checksums, migration fingerprint, numeric non-empty row counts, pg_dump/psql identity, and exported-snapshot/exact-row-count consistency are all release evidence.
- Preserve the producer's exact `${UPLOAD_PREFIX}/${BACKUP_BASENAME}` key construction, including configured leading/trailing separators. The verifier must not normalize a prefix into a namespace the producer did not write.
- Keep scheduler provenance separate from object validity. A manual producer run creates the same trio, so the verifier may prove remote completeness, integrity, freshness, retention, and candidate identity but must not claim scheduled provenance without separate scheduler logs.
- Credentials stay in the AWS credential chain. Provider stderr and command arguments are not retained in errors/reports; the verifier performs only list and download operations and never mutates remote storage.

### Risks and evidence boundaries

- A fresh checksum-valid trio is insufficient if canonical restore would reject its manifest or if it belongs to another release. Require the full existing manifest contract and mandatory candidate identity before `status=pass`.
- Remote object retention is operator/storage policy; the repository verifier proves only a configured minimum complete-set count and does not prune S3.
- A passing report is not restore proof, scheduler provenance, provider durability-SLA evidence, alert-delivery evidence, host-state recovery, or stable promotion. Existing recovery evidence remains separate.
- Production-like completion still requires a real scheduled endpoint: multiple scheduled cycles, visible non-zero remote-failure behavior, local-copy-loss re-verification from remote bytes, retained scheduler provenance, and the resulting secret-safe report. Repository tests cannot manufacture that proof.
- No Compose, root `.env`, generated OME config, permanent CI workflow, backup producer, restore implementation, or remote object mutation belongs in this slice.

### Test and rollout plan

- Deterministic Python tests cover happy path, RPO staleness, incomplete sets, retention minimum, checksum corruption, complete canonical manifest shape, migration/row-count/tool/consistency refusal, exact candidate identity, producer prefix-separator parity, atomic reporting, and secret-safe AWS errors.
- A Go wrapper runs the Python suite through normal Linux/macOS/Windows repository Go gates without adding a permanent workflow.
- Run docs consistency and literal `./scripts/verify.sh` through protected CI, resolve automated review, require the exact-head aggregate Merge gate, and squash merge only after every review conversation is resolved.
- Keep #1299 open after the repository tool lands until live production-like scheduled/off-host evidence is executed and retained against an actual remote endpoint.

### Process note

- Implementation began before this PLAN/TASKS record because the active connector initially exposed only whole-file replacement for these very large ledgers. Automated review correctly flagged the workflow violation. The ledgers are amended before merge, and no exception to the required workflow is claimed for future scoped changes.
